...
namespace util
{
	...
	public:
		...
		ObjectPool(uint cap = 10, GameObject* inst = new GameObject()) { setCapacity(cap); m_inst = inst; }
		...
		{
			capacity = util::clamp(1u,cap,cap);
			count = 0;
			capacity < objects.size() ? objects.resize(cap) : void();
		}
		...
		GameObject* getNewObject()
		{
			...
			if(count >= objects.size())
				objects.push_back(m_inst ? new GameObject(*m_inst) : new GameObject());
